"""
    Version 2.0.
    This program is used to calculate dynamics of with driving force /
    from external field current or magnetic acting on 2D YBCO stack/coil.
    @Author:Brandonlee Santos
    Copyright (C) 2023 Canyon Magnet Energy LLC 
    Implementation largely based upon the following papers/code by Dr. Lingfeng Lai:
    https://iopscience.iop.org/article/10.1088/1361-6668/abc567/meta
    https://iopscience.iop.org/article/10.1088/1361-6668/ac55f4/meta
    https://www.htsmodelling.com/?page_id=748 
...
class Coil:
    ...
    """
    def __init__(self, rin=0.1, rout=0.2, zmin=0.1, count_turns=10,r_in_real = 50e-3,r_out_real=30e-3):
        """
        define the geometry of the coil/stack
        :param rin:  starting unit cell positon
        :param rout:  thickness of unit cell
        :param zmin:  the bottom position of the coil
        :param count_turns:  total turn of the coil
        :r_in_real: the real inner radius
        :r_out_real: the real outer radius
        """
        self.rin = rin
        self.rout = rout
        self.zmin = zmin
        self.count_turns = count_turns
        self.r_in_real = r_in_real
        self.r_out_real = r_out_real
...
class AClossFT2D:
    ...
    def add_coil(self, rin=0.1, rout=0.2, zmin=0.0, count_turns=10,r_in_real=50e-3,r_out_real=30e-3):
        """
        adding a coil in the system
        :param rin: starting unit position
        :param rout: ending unit position
        :param zmin:   the bottom position of the coil
        :param count_turns:  the turn number of the coil
        :param r_in_real: the real inner diameter
        :param r_out_real: the real outer diameter
        :return: None
        """
        self.coils.append(Coil(rin, rout, zmin, count_turns,r_in_real=r_in_real,r_out_real=r_out_real))
    ...
    def run(self, build_matrix=True, cal_jcb=True):
        ...
        while self.time_point_no < len(self.save_time):
            ...
            if reach_save:
                ...
                reach_save = False
        self.cumdta = np.array(self.dta).cumsum()
    ...
    def plot_resultj(self, timestep_n0, use_sym=False, plot3D=False):
        ...
        x = self.posi_x.reshape((self.count_turns, self.ny)) + self.coils[0].r_in_real
        ...
        if plot3D:
            ...
            plotme = ax.contourf
        k0 = 0
        ax.set_xlim(left=self.coils[0].r_in_real, right=self.coils[0].r_out_real)
        ax.set_xlabel('x (m)')
        ax.set_ylabel('y (m)')
        ax.set_title(f'Plot of J/Jc for {len(self.coils)} Coils @ {self.cumdta[timestep_n0]}(sec)' )
        ...
        f.savefig(f'{len(self.coils)}Coils{self.cumdta[timestep_n0]}(sec)CurrentDensityPlot.png')
    ...
    def plot_resultb(self, timestep_n0,  use_sym=False):
        ...
        ax.set_xlabel('x (m)')
        ax.set_ylabel('y (m)')
        ax.set_title(f'Plot of B (T) for {len(self.coils)} Coils @ {self.cumdta[timestep_n0]}(sec)' )
        k0 = 0
        for i in range(len(self.coils)):
            k1 = k0 + self.coils[i].count_turns
        ...
        if use_sym:
            ...
            if abs(self.sym[0] * self.sym[1]) == 1:
                ...
                for i in range(len(self.coils)):
                    ...
                    surf = ax.contourf(-x[k0: k1, :], -y[k0: k1, :], b[k0: k1, :], cmap=cm.coolwarm)
                    k0 = k1
        plt.savefig(f'{len(self.coils)}Coils{self.cumdta[timestep_n0]}(sec)BFieldPlot.png')
        plt.show()
    ...
    def post_cal_power(self, timestep0, timestep1, amp=1):
        ...
        axe.grid(which='both')
        #axe.legend()
        plt.savefig(f'{len(self.coils)}Coils {timestep1} AClossFieldPlot.png')
        plt.show()
...
# r_in_real has to be set 0 or else incorrect physics, this model requires modification
r_inreal=100e-3
r_outreal = 133e-3
heightoft = 4.4e-3
cellthickness = r_outreal/count_turns
tf = 2000
solver.add_coil(1e-4,  1e-4 + cellthickness * count_turns, 0,count_turns,r_in_real = r_inreal,r_out_real=r_outreal)
solver.add_coil(1e-4, 1e-4+ cellthickness * count_turns, heightoft,count_turns,r_in_real = r_inreal,r_out_real=r_outreal)
solver.add_coil(1e-4,1e-4 + cellthickness * count_turns, heightoft*2, count_turns,r_in_real = r_inreal,r_out_real=r_outreal)
solver.add_coil(1e-4, 1e-4+cellthickness * count_turns,heightoft*3, count_turns,r_in_real = r_inreal,r_out_real=r_outreal)
solver.add_coil(1e-4, 1e-4+cellthickness * count_turns, heightoft*4, count_turns,r_in_real = r_inreal)
solver.add_coil(1e-4, 1e-4+cellthickness * count_turns, heightoft*5, count_turns,r_in_real = r_inreal)
solver.add_coil(1e-4, 1e-4+cellthickness * count_turns,heightoft*6, count_turns,r_in_real = r_inreal)
solver.mesh(15)
...
solver.plot_resultj(1000)
solver.plot_resultj(1200)
solver.plot_resultj(1300)
solver.plot_resultj(1400)
solver.plot_resultj(1500)
solver.plot_resultj(1600)
solver.plot_resultj(1700)
solver.plot_resultj(1800)
solver.plot_resultj(1900)
solver.plot_resultb(1000)
solver.plot_resultb(1200)
solver.plot_resultb(1300)
solver.plot_resultb(1400)
solver.plot_resultb(1500)
solver.plot_resultb(1600)
solver.plot_resultb(1700)
solver.plot_resultb(1800)
solver.plot_resultb(1900)
power_avg = solver.post_cal_power(1,tf-1)
print(f' ACLOSS {sum(power_avg)}')
plt.plot()
